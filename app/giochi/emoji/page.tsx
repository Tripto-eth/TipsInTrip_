...
import { getEmojiRound, type EmojiRound } from '../../lib/emojiPuzzles';
...
function GlobalLeaderboard({ game }: { game: string }) {
  const { t } = useLang();
  const tg = t.games;
...
// Category hint descriptions shown in menu
const DIFF_CATEGORIES: Record<Difficulty, string> = {
  easy:   '🌍 Paesi · 🏙️ Città · 🍽️ Cibo',
  medium: '🌍 Paesi · 🏙️ Città · 🍽️ Cibo',
  hard:   '🌍 Paesi · 🏙️ Città · ✈️ Compagnie',
};
...
export default function EmojiPage() {
  ...
  const [rounds, setRounds] = useState<EmojiRound[]>([]);
  ...
  useEffect(() => {
    ...
    saveScore(entry, 'emoji' as Game);
    setLocalBoard(getLeaderboard('emoji' as Game));
    if (isSignedIn && !saved) {
      setSaving(true);
      fetch('/api/quiz/score', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ score, correct, total: rounds.length, diff, game: 'emoji' }),
      })
        .then(() => setSaved(true))
        .catch(() => {})
    ...
    <main style={pageStyle}>
      ...
      <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
        ...
        <p style={{ fontSize: '0.88rem', color: 'rgba(255,255,255,0.5)', margin: 0 }}>
          Tre emoji descrivono una destinazione, compagnia o piatto. Indovina!
        </p>
      </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.7rem' }}>
                ...
                <div style={{ textAlign: 'left' }}>
                  ...
                  <div style={{ fontSize: '0.72rem', opacity: 0.5 }}>
                    {DIFF_CATEGORIES[d]} · {c.points} {tg.ptPerQ} · {c.time}{tg.secPerQ}
                  </div>
                </div>
              </div>
    ...
    return (
      <main style={pageStyle}>
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          ...
          <p style={{ color: 'rgba(255,255,255,0.5)', margin: '0 0 0.5rem' }}>{correct} {tg.correctOf} {rounds.length} {tg.correctSuffix} — {pct}%</p>
          <DiffBadge diff={diff} />
          <div style={{ marginTop: '0.75rem', fontSize: '0.8rem', color: 'rgba(255,255,255,0.4)' }}>
            {saving && tg.saving}{saved && tg.saved}
          </div>
        </div>
  ...
  // ── PLAYING / FEEDBACK ────────────────────────────────────────────────────
  if (!round) return <main style={pageStyle}><div style={{ textAlign: 'center', padding: '3rem', color: 'rgba(255,255,255,0.4)' }}>{tg.loading}</div></main>;

  return (
    <main style={pageStyle}>
      <style>{`
        @keyframes emojiPop { from { opacity:0; transform:scale(0.6); } to { opacity:1; transform:scale(1); } }
        .emoji-anim { animation: emojiPop 0.4s cubic-bezier(0.175,0.885,0.32,1.275) both; }
      `}</style>
      ...
      {/* Categoria + emoji */}
      <div className="emoji-anim" key={qi} style={{ textAlign: 'center', margin: '1.5rem 0 2rem' }}>
        <div style={{ marginBottom: '0.75rem' }}>
          <span style={{ padding: '4px 12px', borderRadius: 999, background: 'rgba(157,78,221,0.18)', border: '1px solid rgba(157,78,221,0.35)', fontSize: '0.78rem', color: PURPLE_LIGHT, fontWeight: 600 }}>
            {round.categoryLabel}
          </span>
        </div>
        <div style={{ fontSize: 'clamp(3.5rem,15vw,6rem)', lineHeight: 1.2, letterSpacing: '0.1em', filter: 'drop-shadow(0 6px 18px rgba(0,0,0,0.5))' }}>
          {round.emoji}
        </div>
        <p style={{ fontSize: '0.82rem', color: 'rgba(255,255,255,0.35)', marginTop: '0.6rem' }}>
          Di cosa si tratta?
        </p>
      </div>
